...
use audio::{
    ...
    AudioStream, AudioDevice, DeviceType, encode_single_audio,
};
...
// MultiWriter struct to write logs to both file and console
...
impl<W1: Write, W2: Write> Write for MultiWriter<W1, W2> {
    fn write(&mut self, buf: &[u8]) -> IoResult<usize> {
        ...
        let _ = self.writer1.write(buf);
        let _ = self.writer2.write(buf);
        Ok(buf.len())
    }

    fn flush(&mut self) -> IoResult<()> {
        let _ = self.writer1.flush();
...
// Audio configuration constants - OPTIMIZED FOR REAL-TIME RESPONSE
const CHUNK_DURATION_MS: u32 = 1000; // 1 second per chunk for real-time response
...
const SENTENCE_TIMEOUT_MS: u64 = 300; // Emit incomplete sentence after 300ms for real-time feel
const MIN_CHUNK_DURATION_MS: u32 = 250; // Minimum 250ms chunks for responsiveness
const PARTIAL_EMIT_INTERVAL_MS: u64 = 100; // Emit partial results every 100ms
...
struct TranscriptAccumulator {
    ...
    last_partial_emit_time: std::time::Instant,
    last_emitted_partial_text: String,
}
...
impl TranscriptAccumulator {
    fn new() -> Self {
        ...
        Self {
            ...
            last_emitted_partial_text: String::new(),
        }
    }
    ...
    fn add_segment(&mut self, segment: &TranscriptSegment) -> Option<TranscriptUpdate> {
        ...
        if clean_text.is_empty() {
            log_info!("Skipping segment with empty text");
            return None;
        }
        
        if (segment.t1 - segment.t0) < 1.0 {
            log_info!("Skipping segment too short: {}s duration (text: '{}')", segment.t1 - segment.t0, clean_text);
            return None;
        }
        ...
        if has_sentence_ending {
            ...
            // Clear the last emitted partial text since we're completing the sentence
            self.last_emitted_partial_text.clear();
    ...
    fn check_timeout(&mut self) -> Option<TranscriptUpdate> {
        if !self.current_sentence.is_empty() && 
           self.last_update_time.elapsed() > Duration::from_millis(SENTENCE_TIMEOUT_MS) {
            ...
            // Clear the last emitted partial text since we're timing out the sentence
            self.last_emitted_partial_text.clear();
    ...
    /// Check if we should emit partial results for real-time streaming
    fn check_partial_emit(&mut self) -> Option<TranscriptUpdate> {
        // Only emit partials if we have meaningful content, enough time has passed, and content has changed
        let current_text = self.current_sentence.trim();
        
        if !current_text.is_empty() && 
           current_text.len() > 5 && // At least 5 characters to avoid noise
           current_text != self.last_emitted_partial_text &&
           self.last_partial_emit_time.elapsed() > Duration::from_millis(500) { // Increased from 100ms to 500ms
            
            let sequence_id = SEQUENCE_COUNTER.fetch_add(1, Ordering::SeqCst);
            self.last_partial_emit_time = std::time::Instant::now();
            self.last_emitted_partial_text = current_text.to_string();
            
            // Calculate elapsed time for partial result
            let start_elapsed = if let Some(recording_start) = self.recording_start_time {
...
async fn audio_collection_task<R: Runtime>(
    ...
    let mut system_receiver = match &system_stream {
        Some(stream) => match stream.subscribe().await {
            Ok(receiver) => Some(receiver),
            Err(e) => {
                log_error!("Failed to subscribe to system audio stream: {}", e);
                None
            }
        },
        None => {
    ...
    while is_running.load(Ordering::SeqCst) {
        ...
        // 🔄 System Audio Reconnection Logic
        if system_receiver.is_none() && system_stream.is_some() {
            let now = std::time::Instant::now();
            // Only attempt reconnection every 5 seconds to avoid spam
            if now.duration_since(last_reconnection_attempt).as_secs() >= 5 {
                ...
                // Subscribe returns Result now, handle it properly
                match system_stream.as_ref().unwrap().subscribe().await {
                    Ok(new_receiver) => {
                    ...
                    Err(e) => {
                        log_error!("Failed to resubscribe to system audio: {}", e);
                        system_audio_failure_count += 1;
                    }
                }
                
                // Emit reconnection success event
                if let Err(e) = app_handle.emit("system-audio-reconnected", serde_json::json!({
        ...
        if mic_chunks_received == 0 {
            ...
            unsafe {
                ...
                if current_time - LAST_MIC_RECOVERY_ATTEMPT >= 10 {
                    ...
                    // Try to recreate the microphone receiver
                    match mic_stream.subscribe().await {
                        Ok(new_receiver) => {
                        ...
                        Err(e) => {
                            log_error!("Failed to resubscribe to microphone: {}", e);
                            // Keep failure count, don't reset
                        }
                    }
                    
                    // Emit recovery event
                    if let Err(e) = app_handle.emit("microphone-recovered", serde_json::json!({
                        "message": "Microphone capture restored",
        ...
        if should_create_chunk && !current_chunk.is_empty() {
            ...
            if chunk_rms < CHUNK_SILENCE_THRESHOLD * 0.3 && chunk_avg_level < CHUNK_AVG_SILENCE_THRESHOLD * 0.3 {
                ...
                log_info!("🔇 Quick silence detection - skipping chunk: RMS: {:.6}, Avg: {:.6} (well below thresholds)", 
            ...
            log_info!("📊 Audio chunk ready: {} samples (VAD pre-processed, speech content confirmed)", whisper_samples.len());
...
async fn transcribe_audio_chunk_whisper_rs(chunk: Vec<f32>) -> Result<TranscriptResponse, String> {
    ...
    unsafe {
        if let Some(engine) = &WHISPER_ENGINE {
            ...
            log_info!("Whisper model is loaded, resampling audio...");
        ...
        log_info!("Audio levels - Max: {:.6}, Avg: {:.6}", max_amplitude, avg_amplitude);
...
async fn whisper_rs_transcription_worker<R: Runtime>(
    ...
    loop {
        ...
        // Check for progressive partial emission (real-time streaming)
        if let Some(update) = accumulator.check_partial_emit() {
            log_info!("Whisper-rs Worker {}: Emitting partial transcript-update event with sequence_id: {}", worker_id, update.sequence_id);
            
            if let Err(e) = app_handle.emit("transcript-update", &update) {
                log_error!("Whisper-rs Worker {}: Failed to send partial transcript update: {}", worker_id, e);
            } else {
                log_info!("Whisper-rs Worker {}: Successfully emitted partial transcript-update event", worker_id);
            }
        }
...
async fn start_recording<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    ...
    let (use_local_whisper, whisper_model) = match transcript_config_result {
        ...
        Err(e) => {
            log_info!("❌ Could not get transcript config, defaulting to HTTP: {}", e);
            (false, "small".to_string())
        }
    };
    ...
    // Create HTTP client for transcription (only if not using local whisper)
    let client = reqwest::Client::new();
    
    // Use hardcoded transcript server URL (only for HTTP-based providers)
    ...
    // Start transcription workers based on provider
...
fn create_wav_file_from_buffers() -> Result<Option<Vec<u8>>, String> {
    use hound::{WavSpec, WavWriter};
    use std::io::Cursor;
    
    // Get audio data from global buffers
    let mic_data = unsafe {
    ...
    // If no audio data was captured, return None
    if mic_data.is_empty() && system_data.is_empty() {
        log_info!("No audio data to save");
        return Ok(None);
    }
    
    log_info!("Creating WAV file from {} mic samples and {} system samples", 
             mic_data.len(), system_data.len());
    
    // Use the larger buffer size for final output
    let total_samples = std::cmp::max(mic_data.len(), system_data.len());
    let mut mixed_audio = Vec::with_capacity(total_samples);
    ...
    for i in 0..total_samples {
        ...
        // Simple mixing - you can adjust gains as needed
        let mixed_sample = (mic_sample * 0.7 + system_sample * 0.8).clamp(-1.0, 1.0);
    ...
    log_info!("Successfully created WAV file with {} bytes", wav_data.len());
...
async fn stop_recording(args: RecordingArgs) -> Result<(), String> {
    ...
    */
    ...
    // Create WAV file from collected audio data
    let wav_file = create_wav_file_from_buffers()?
        .ok_or_else(|| "No audio data recorded".to_string())?;
    
    log_info!("Created WAV file with {} bytes", wav_file.len());
    
    // Save the recording
    log_info!("Saving recording to: {}", args.save_path);
    match std::fs::write(&args.save_path, wav_file) {
        Ok(_) => {
            log_info!("Successfully saved recording to: {}", args.save_path);
            
            // Show the saved file location in Finder/Explorer
            #[cfg(target_os = "macos")]
            {
                let _ = std::process::Command::new("open")
                    .args(["-R", &args.save_path])
                    .spawn();
            }
...
pub fn run() {
    tauri::Builder::default()
        .setup(|_app| {
            ...
            #[cfg(target_os = "macos")]
            {
                // Initialize file logging with timestamps
            ...
            log::info!("Application setup complete - logging to both console and tauri-app.log");
            ...
            // Request Screen Recording permission for system audio (macOS only)
            #[cfg(target_os = "macos")]
            {
                if let Err(e) = audio::core::request_screen_recording_permission() {
                    log::warn!("Screen Recording permission not available: {}", e);
                } else {
                    log::info!("Screen Recording permission check completed");
                }
            }
